...
class SimulationAgent:
    def __init__(
        ...
        self.state = state  # WALKING, QUEUING, WAITING, ENTERING, EXITING, EVACUATING, REROUTING, BLOCKED, PANIC, SAFE
        self.zone = zone
        ...
        # Physical parameters for Social Force
    ...
    def trigger_emergency(self, evacuation_target: Optional[Tuple[float, float]] = None, panic: float = 1.0):
        if self.state != "SAFE":
            ...
            self.desired_speed = random.uniform(3.2, 4.5)  # rapid sprint / stampede speed
    ...
    def update_social_forces(
        ...
        incorporating Panic Contagion, High-Density Compression Waves, and Stampede dynamics.
        """
        if self.state == "SAFE":
            return

        # 0. Check Local Density & Panic Contagion
        nearby_count = 0
        max_nearby_panic = 0.0
        for other in nearby_agents:
            if other.id != self.id and other.state != "SAFE":
                d = math.hypot(self.x - other.x, self.y - other.y)
                if d < 2.5:
                    nearby_count += 1
                    max_nearby_panic = max(max_nearby_panic, other.panic_level)

        # Local density in agents/m² (approx area ~ 19.6 m²)
        ...
        if max_nearby_panic > 0.4 and self.panic_level < max_nearby_panic:
            ...
            if self.panic_level > 0.6 and self.state not in ["PANIC", "SAFE"]:
                self.state = "PANIC"
                self.desired_speed = random.uniform(3.4, 4.6)

        # Proximity to active fire / hazard danger zones triggers high panic & avoidance
        f_hazard_x = 0.0
        f_hazard_y = 0.0
        if danger_zones:
            for dz in danger_zones:
                ...
                if dist < r * 1.5:
                    ...
                    # Repulsive push away from hazard
                    d_safe = max(0.1, dist)
                    push = 4000.0 * (1.0 - min(1.0, dist / (r * 1.5)))
                    f_hazard_x += (dx / d_safe) * push
                    f_hazard_y += (dy / d_safe) * push

        # 1. Driving Force towards current waypoint or emergency exit gradient
        is_stampeding = is_emergency or self.state in ["EVACUATING", "PANIC"] or self.panic_level > 0.5

        if is_stampeding:
            # Direct gradient query from navmesh Dijkstra distance field
        ...
        # If in high-density bottleneck, velocity is constrained by physical crowd density
        ...
        # 2. Inter-agent repulsion & Helbing Physical Contact Force
        f_agents_x = 0.0
        f_agents_y = 0.0
        A = 2600.0 if is_stampeding else 1400.0
        B = 0.16
        ...
        for other in nearby_agents:
            ...
            if dist < 2.5:
                ...
                if overlap > 0:
                    ...
                    repulsion += 16000.0 * overlap
                    # Tangential sliding friction
                    t_x = -n_y
                    t_y = n_x
                    delta_v = (other.vx - self.vx) * t_x + (other.vy - self.vy) * t_y
                    f_agents_x += 1200.0 * overlap * delta_v * t_x
                    f_agents_y += 1200.0 * overlap * delta_v * t_y
